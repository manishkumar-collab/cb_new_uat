...
// Get statistics - Modified to get separate totals for approved requests with GST and without GST
 $stats_sql = "SELECT 
         ...
         SUM(cr.premium_with_gst) AS total_premium_all_requests,
         SUM(cr.referral_amount) AS total_referral_amount,
         ...
         SUM(cr.without_gst) AS total_without_gst_all_requests,
...
// Calculate total premium (head approved + validator approved + finance approved)
 $total_premium_with_gst = $stats['head_premium_with_gst'] + $stats['validator_premium_with_gst'] + $stats['finance_premium_with_gst'];
 $total_premium_without_gst = $stats['head_without_gst'] + $stats['validator_without_gst'] + $stats['finance_without_gst'];

// Calculate total cashback (head approved + validator approved + finance approved)
 $total_cashback = $stats['approved_amount'] + $stats['validator_approved_amount'] + $stats['finance_approved_amount'];

// Calculate cashback as a percentage of total premium with GST
 $cashback_percentage_with_gst_display = "N/A";
if ($total_premium_with_gst > 0) {
    $cashback_percentage_with_gst = ($total_cashback / $total_premium_with_gst) * 100;
    $cashback_percentage_with_gst_display = number_format($cashback_percentage_with_gst, 2) . "%";
}

// Calculate cashback as a percentage of total premium without GST
 $cashback_percentage_without_gst_display = "N/A";
if ($total_premium_without_gst > 0) {
    $cashback_percentage_without_gst = ($total_cashback / $total_premium_without_gst) * 100;
    $cashback_percentage_without_gst_display = number_format($cashback_percentage_without_gst, 2) . "%";
}

// Calculate CB to Shortfall ratio
 $cb_shortfall_ratio_display = "N/A";
if ($stats['total_shortfall_amount'] > 0) {
    $cb_shortfall_ratio = ($stats['total_cb_amount'] / $stats['total_shortfall_amount']);
    $cb_shortfall_ratio_display = number_format($cb_shortfall_ratio, 2);
}

// Calculate shortfall percentage of total premium with GST
 $shortfall_percentage_with_gst_display = "N/A";
if ($total_premium_with_gst > 0) {
    $shortfall_percentage_with_gst = ($stats['total_shortfall_amount'] / $total_premium_with_gst) * 100;
    $shortfall_percentage_with_gst_display = number_format($shortfall_percentage_with_gst, 2) . "%";
}

// Calculate shortfall percentage of total premium without GST
 $shortfall_percentage_without_gst_display = "N/A";
if ($total_premium_without_gst > 0) {
    $shortfall_percentage_without_gst = ($stats['total_shortfall_amount'] / $total_premium_without_gst) * 100;
...
// Get month-wise statistics for new dashboard card - Modified to include without GST
 $month_wise_sql = "SELECT 
                   cr.month,
                   cr.year,
                   SUM(CASE WHEN cr.form_type = 'CB' THEN cr.premium_with_gst ELSE 0 END) AS cb_premium_with_gst,
                   SUM(CASE WHEN cr.form_type = 'Shortfall' THEN cr.premium_with_gst ELSE 0 END) AS shortfall_premium_with_gst,
                   SUM(CASE WHEN cr.form_type = 'CB' THEN cr.without_gst ELSE 0 END) AS cb_premium_without_gst,
                   SUM(CASE WHEN cr.form_type = 'Shortfall' THEN cr.without_gst ELSE 0 END) AS shortfall_premium_without_gst,
                   SUM(CASE WHEN cr.form_type = 'CB' THEN cr.referral_amount ELSE 0 END) AS cb_amount,
                   SUM(CASE WHEN cr.form_type = 'Shortfall' THEN cr.referral_amount ELSE 0 END) AS shortfall_amount
                   FROM cashback_requests cr 
                   JOIN users u ON cr.user_id = u.id 
            ...
            <div class="container">
                ...
                <div class="dashboard-container">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            <div class="stat-value">₹<?php echo number_format($stats['validator_approved_amount'], 2); ?></div>
                        ...
                        <div class="stat-card">
                            <div class="stat-value">₹<?php echo number_format($stats['finance_approved_amount'], 2); ?></div>
                            <div class="stat-label">Finance Approved Amount</div>
                        </div>
                        <div class="stat-card">
                            <div class="stat-value">₹<?php echo number_format($stats['finance_rejected_amount'], 2); ?></div>
                            <div class="stat-label">Finance Rejected Amount</div>
                        </div>
                        <div class="stat-card">
                            <div class="stat-value">₹<?php echo number_format($stats['validator_rejected_amount'], 2); ?></div>
                    ...
                    <div class="cashback-stats">
                        <div class="cashback-stat-card">
                            <div class="cashback-stat-value">₹<?php echo number_format($total_premium_with_gst, 2); ?></div>
                            <div class="cashback-stat-label">Total Premium (with GST)</div>
                        </div>
                        <div class="cashback-stat-card">
                            <div class="cashback-stat-value">₹<?php echo number_format($stats['total_cb_amount'], 2); ?></div>
                        ...
                        <div class="cashback-stat-card">
                            <div class="cashback-stat-value">₹<?php echo number_format($stats['total_shortfall_amount'], 2); ?></div>
                      ...
                      <!--  <div class="cashback-stat-card">
                            <div class="cashback-stat-value"><?php echo $cb_shortfall_ratio_display; ?></div>
                            <div class="cashback-stat-label">CB to Shortfall Ratio</div>
                        </div> -->
                    </div>
                    <div class="cashback-stats">
                        <div class="cashback-stat-card">
                            <div class="cashback-stat-value">₹<?php echo number_format($total_premium_without_gst, 2); ?></div>
                    ...
                    <div class="month-wise-card">
                        ...
                        <!-- Third Section: CB to Shortfall Ratio -->
                     <!--   <h4 style="margin: 20px 0 15px; color: var(--primary); font-size: 15px;">CB to Shortfall Ratio</h4>
                        <div class="cashback-stats">
                            <div class="cashback-stat-card">
                                <div class="cashback-stat-value" id="cb-to-shortfall-ratio">0</div>
                                <div class="cashback-stat-label">CB to Shortfall Ratio</div>
                            </div>
                        </div>
                    </div> -->
                            ...
                            <div class="hierarchy-grid">
                                ...
                                while ($manager = mysqli_fetch_assoc($managers_result)): ?>
                                    ...
                                    $pending_count_sql = "SELECT COUNT(*) AS count 
                                                       ...
                                                       JOIN users u ON cr.user_id = u.id 
                                                       JOIN ($status_subquery) s ON cr.id = s.id
                                                       WHERE u.manager_id = ? AND s.actual_status = 'Manager Approved'";
                                    ...
                                    $team_amount_sql = "SELECT SUM(cr.referral_amount) AS amount 
                                                     ...
                                                     JOIN users u ON cr.user_id = u.id 
                                                     JOIN ($status_subquery) s ON cr.id = s.id
                                                     WHERE u.manager_id = ? AND (s.actual_status = 'Head Approved' OR s.actual_status = 'Validator Approved' OR s.actual_status = 'Finance Approved')";
                    ...
                    <div class="analytics-grid">
                        <!-- Department-wise Analytics commented out -->
                        <!--
                                ...
                                <table class="analytics-table">
                                    ...
                                    <tbody>
                                        <?php while ($dept = mysqli_fetch_assoc($dept_result)): ?>
                                            ...
                                            // Calculate CB ratio
                                            $cb_ratio = 0;
                                            if ($dept['premium_with_gst'] > 0) {
                                                $cb_ratio = ($dept['amount'] / $dept['premium_with_gst']) * 100;
                                            }
                                            ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($dept['department']); ?></td>
                                                <td><?php echo $dept['count']; ?></td>
                                                <td>₹<?php echo number_format($dept['premium_with_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($dept['without_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($dept['amount'], 2); ?></td>
                        ...
                        <div class="analytics-card">
                            ...
                            <div class="table-container">
                                <table class="analytics-table">
                                    ...
                                    <tbody>
                                        <?php while ($manager = mysqli_fetch_assoc($manager_stats_result)): ?>
                                            <?php 
                                            // Calculate CB ratio
                                            $cb_ratio = 0;
                                            if ($manager['cb_premium_with_gst'] > 0) {
                                                $cb_ratio = ($manager['cb_amount'] / $manager['cb_premium_with_gst']) * 100;
                                            }
                                            
                                            // Calculate Shortfall ratio
                                            $shortfall_ratio = 0;
                                            if ($manager['shortfall_premium_with_gst'] > 0) {
                                                $shortfall_ratio = ($manager['shortfall_amount'] / $manager['shortfall_premium_with_gst']) * 100;
                                            }
                                            ?>
                                            <tr>
                                                ...
                                                <td>₹<?php echo number_format($manager['premium_with_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($manager['without_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($manager['cb_amount'], 2); ?></td>
                                                <td>₹<?php echo number_format($manager['shortfall_amount'], 2); ?></td>
                                                <td><?php echo number_format($cb_ratio, 2); ?>%</td>
                        ...
                        <div class="analytics-card">
                            ...
                            <div class="table-container">
                                <table class="analytics-table">
                                    ...
                                    <tbody>
                                        <?php while ($team = mysqli_fetch_assoc($team_stats_result)): ?>
                                            <?php 
                                            // Calculate CB ratio
                                            $cb_ratio = 0;
                                            if ($team['cb_premium_with_gst'] > 0) {
                                                $cb_ratio = ($team['cb_amount'] / $team['cb_premium_with_gst']) * 100;
                                            }
                                            
                                            // Calculate Shortfall ratio
                                            $shortfall_ratio = 0;
                                            if ($team['shortfall_premium_with_gst'] > 0) {
                                                $shortfall_ratio = ($team['shortfall_amount'] / $team['shortfall_premium_with_gst']) * 100;
                                            }
                                            ?>
                                            <tr>
                                                ...
                                                <td>₹<?php echo number_format($team['premium_with_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($team['without_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($team['cb_amount'], 2); ?></td>
                                                <td>₹<?php echo number_format($team['shortfall_amount'], 2); ?></td>
                                                <td><?php echo number_format($cb_ratio, 2); ?>%</td>
                                                <td><?php echo number_format($shortfall_ratio, 2); ?>%</td>
                                            </tr>
                                        <?php endwhile; ?>
                        ...
                        <div class="analytics-card">
                            ...
                            <div class="table-container">
                                <table class="analytics-table">
                                    ...
                                    <tbody>
                                        <?php while ($policy_type = mysqli_fetch_assoc($policy_type_stats_result)): ?>
                                            <?php 
                                            // Calculate CB ratio
                                            $cb_ratio = 0;
                                            if ($policy_type['cb_premium_with_gst'] > 0) {
                                                $cb_ratio = ($policy_type['cb_amount'] / $policy_type['cb_premium_with_gst']) * 100;
                                            }
                                            
                                            // Calculate Shortfall ratio
                                            $shortfall_ratio = 0;
                                            if ($policy_type['shortfall_premium_with_gst'] > 0) {
                                                $shortfall_ratio = ($policy_type['shortfall_amount'] / $policy_type['shortfall_premium_with_gst']) * 100;
                                            }
                                            ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($policy_type['policy_type']); ?></td>
                                                <td><?php echo $policy_type['count']; ?></td>
                                                <td>₹<?php echo number_format($policy_type['premium_with_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($policy_type['without_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($policy_type['cb_amount'], 2); ?></td>
                                                <td>₹<?php echo number_format($policy_type['shortfall_amount'], 2); ?></td>
                                                <td><?php echo number_format($cb_ratio, 2); ?>%</td>
                                                <td><?php echo number_format($shortfall_ratio, 2); ?>%</td>
                                            </tr>
                                        <?php endwhile; ?>
                        ...
                        <div class="analytics-card">
                            ...
                            <div class="table-container">
                                <table class="analytics-table">
                                    ...
                                    <tbody>
                                        <?php while ($insurance_company = mysqli_fetch_assoc($insurance_company_stats_result)): ?>
                                            <?php 
                                            // Calculate CB ratio
                                            $cb_ratio = 0;
                                            if ($insurance_company['cb_premium_with_gst'] > 0) {
                                                $cb_ratio = ($insurance_company['cb_amount'] / $insurance_company['cb_premium_with_gst']) * 100;
                                            }
                                            
                                            // Calculate Shortfall ratio
                                            $shortfall_ratio = 0;
                                            if ($insurance_company['shortfall_premium_with_gst'] > 0) {
                                                $shortfall_ratio = ($insurance_company['shortfall_amount'] / $insurance_company['shortfall_premium_with_gst']) * 100;
                                            }
                                            ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($insurance_company['insurance_company']); ?></td>
                                                <td><?php echo $insurance_company['count']; ?></td>
                                                <td>₹<?php echo number_format($insurance_company['premium_with_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($insurance_company['without_gst'], 2); ?></td>
                                                <td>₹<?php echo number_format($insurance_company['cb_amount'], 2); ?></td>
                                                <td>₹<?php echo number_format($insurance_company['shortfall_amount'], 2); ?></td>
                                                <td><?php echo number_format($cb_ratio, 2); ?>%</td>
                                                <td><?php echo number_format($shortfall_ratio, 2); ?>%</td>
                                            </tr>
                                        <?php endwhile; ?>
                    ...
                    <div id="pending-tab" class="tab-content active">
                        ...
                        <div class="filter-container">
                            ...
                            <div class="filter-group">
                                ...
                                <select id="manager-filter" name="manager">
                            ...
                            <div class="filter-group">
                                <label for="policy-type-filter">Policy Type:</label>
                                <select id="policy-type-filter" name="policy_type">
                            ...
                            <div class="filter-group">
                                ...
                                <select id="insurance-company-filter" name="insurance_company">
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        <?php while ($request = mysqli_fetch_assoc($pending_result)): ?>
                                            ...
                                            $time_taken_class = '';
                                            if ($request['manager_time_taken'] > 24) {
                                                $time_taken_class = 'time-taken-negative';
                                            } else {
                                                $time_taken_class = 'time-taken-positive';
                                            }
                                            ?>
                                            <tr>
    ...
    <script>
        ...
        function updateMonthData() {
            ...
            if (selectedMonth && monthWiseData[selectedMonth]) {
                ...
                document.getElementById('cb-premium-with-gst').textContent = '₹' + parseFloat(data.cb_premium_with_gst).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Update CB Amount
                document.getElementById('cb-amount').textContent = '₹' + parseFloat(data.cb_amount).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Calculate and update CB % of Premium with GST
                const cbPercentageWithGst = data.cb_premium_with_gst > 0 ? (data.cb_amount / data.cb_premium_with_gst) * 100 : 0;
                document.getElementById('cb-percentage-with-gst').textContent = cbPercentageWithGst.toFixed(2) + '%';
                
                // Update CB Premium without GST
                document.getElementById('cb-premium-without-gst').textContent = '₹' + parseFloat(data.cb_premium_without_gst).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Update CB Amount (same as with GST)
                document.getElementById('cb-amount-without-gst').textContent = '₹' + parseFloat(data.cb_amount).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Calculate and update CB % of Premium without GST
                const cbPercentageWithoutGst = data.cb_premium_without_gst > 0 ? (data.cb_amount / data.cb_premium_without_gst) * 100 : 0;
                document.getElementById('cb-percentage-without-gst').textContent = cbPercentageWithoutGst.toFixed(2) + '%';
                
                // Update Shortfall Premium with GST
                document.getElementById('shortfall-premium-with-gst').textContent = '₹' + parseFloat(data.shortfall_premium_with_gst).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Update Shortfall Amount
                document.getElementById('shortfall-amount').textContent = '₹' + parseFloat(data.shortfall_amount).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Calculate and update Shortfall % of Premium with GST
                const shortfallPercentageWithGst = data.shortfall_premium_with_gst > 0 ? (data.shortfall_amount / data.shortfall_premium_with_gst) * 100 : 0;
                document.getElementById('shortfall-percentage-with-gst').textContent = shortfallPercentageWithGst.toFixed(2) + '%';
                
                // Update Shortfall Premium without GST
                document.getElementById('shortfall-premium-without-gst').textContent = '₹' + parseFloat(data.shortfall_premium_without_gst).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Update Shortfall Amount (same as with GST)
                document.getElementById('shortfall-amount-without-gst').textContent = '₹' + parseFloat(data.shortfall_amount).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                
                // Calculate and update Shortfall % of Premium without GST
                const shortfallPercentageWithoutGst = data.shortfall_premium_without_gst > 0 ? (data.shortfall_amount / data.shortfall_premium_without_gst) * 100 : 0;
                document.getElementById('shortfall-percentage-without-gst').textContent = shortfallPercentageWithoutGst.toFixed(2) + '%';
                
                // Calculate and update CB to Shortfall Ratio
                const cbToShortfallRatio = data.shortfall_amount > 0 ? (data.cb_amount / data.shortfall_amount) : 0;
                document.getElementById('cb-to-shortfall-ratio').textContent = cbToShortfallRatio.toFixed(2);
            } else {
                ...
                document.getElementById('cb-to-shortfall-ratio').textContent = '0';
            }
        }
        ...
        function applyFilters() {
            ...
            if (formType) {
                url += '&form_type=' + encodeURIComponent(formType);
            }
            ...
            if (policyType) {
                url += '&policy_type=' + encodeURIComponent(policyType);
            }
            
            if (insuranceCompany) {
                url += '&insurance_company=' + encodeURIComponent(insuranceCompany);
            }
