...
// Get validator's rejection reason and approval ID
 $rejection_sql = "SELECT id, comments FROM approvals WHERE request_id = ? AND approver_role = 'Validator' AND status = 'Rejected' ORDER BY created_at DESC LIMIT 1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($justification)) {
        ...
        // Insert justification into user_justifications table
        $insert_sql = "INSERT INTO user_justifications (request_id, user_id, approval_id, justification_text) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $insert_sql);
        mysqli_stmt_bind_param($stmt, "iiis", $request_id, $_SESSION['user_id'], $approval_id, $justification);
        
        if (mysqli_stmt_execute($stmt)) {
            // Create a new approval entry for the validator to review the justification
            $approval_sql = "INSERT INTO approvals (request_id, approver_id, approver_role, status, comments) 
                            SELECT ?, u.id, 'Validator', 'Pending', 'User justification provided for review' 
                            FROM users u WHERE u.role = 'Validator' LIMIT 1";
            $stmt = mysqli_prepare($conn, $approval_sql);
            mysqli_stmt_bind_param($stmt, "i", $request_id);
            mysqli_stmt_execute($stmt);
            
            // Update request status to Pending Validation
            $reset_sql = "UPDATE cashback_requests SET status = 'Pending Validation' WHERE id = ?";
    ...
    <div class="container">
        ...
        <?php if (mysqli_num_rows($check_result) > 0): ?>
            <div class="alert alert-error">
                <i class="fas fa-exclamation-circle"></i> You have already provided a justification for this request. It is currently under review.
            </div>
        <?php else: ?>
