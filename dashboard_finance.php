...
// Get overall statistics with filters (Calculation Fix Included)
 $stats_sql = "SELECT 
              COUNT(*) AS total_requests,
              SUM(CASE WHEN cr.status = 'Validator Approved' THEN 1 ELSE 0 END) AS pending_count,
              SUM(CASE WHEN cr.status = 'Finance Approved' THEN 1 ELSE 0 END) AS approved_count,
              SUM(CASE WHEN cr.status = 'Rejected' OR cr.status = 'Validator Rejected' THEN 1 ELSE 0 END) AS rejected_count,
              SUM(CASE WHEN cr.status = 'Validator Approved' THEN cr.referral_amount ELSE 0 END) AS pending_amount,
              SUM(CASE WHEN cr.status = 'Finance Approved' THEN cr.referral_amount ELSE 0 END) AS approved_amount,
              SUM(CASE WHEN cr.status = 'Rejected' OR cr.status = 'Validator Rejected' THEN cr.referral_amount ELSE 0 END) AS rejected_amount,
              SUM(CASE WHEN cr.status IN ('Validator Approved', 'Finance Approved') THEN cr.premium_with_gst ELSE 0 END) AS total_premium_with_gst,
              SUM(CASE WHEN cr.status IN ('Validator Approved', 'Finance Approved') THEN cr.without_gst ELSE 0 END) AS total_without_gst,
              AVG(CASE WHEN cr.status = 'Finance Approved' THEN cr.referral_amount ELSE NULL END) AS avg_cashback,
              AVG(CASE WHEN cr.status = 'Finance Approved' THEN cr.premium_with_gst ELSE NULL END) AS avg_premium,
              (SUM(CASE WHEN cr.status IN ('Validator Approved', 'Finance Approved') THEN cr.referral_amount ELSE 0 END) / 
              NULLIF(SUM(CASE WHEN cr.status IN ('Validator Approved', 'Finance Approved') THEN cr.premium_with_gst ELSE 0 END), 0)) * 100 AS cashback_ratio_with_gst,
              (SUM(CASE WHEN cr.status IN ('Validator Approved', 'Finance Approved') THEN cr.referral_amount ELSE 0 END) / 
              NULLIF(SUM(CASE WHEN cr.status IN ('Validator Approved', 'Finance Approved') THEN cr.without_gst ELSE 0 END), 0)) * 100 AS cashback_ratio_without_gst
              FROM cashback_requests cr 
              JOIN users u ON cr.user_id = u.id 
              LEFT JOIN users h ON u.head_id = h.id
              WHERE 1=1" . $finance_condition . $filter_conditions;
 $stats_result = mysqli_query($conn, $stats_sql);
 $stats = mysqli_fetch_assoc($stats_result);

// Handle null values for cashback_ratio
if ($stats['cashback_ratio_with_gst'] === null) {
    $stats['cashback_ratio_with_gst'] = 0;
}
if ($stats['cashback_ratio_without_gst'] === null) {
    $stats['cashback_ratio_without_gst'] = 0;
}
...
// Get monthly statistics for the last 12 months
 $monthly_sql = "SELECT 
                ...
                (SUM(cr.referral_amount) / NULLIF(SUM(cr.premium_with_gst), 0)) * 100 AS cashback_ratio_with_gst,
                (SUM(cr.referral_amount) / NULLIF(SUM(cr.without_gst), 0)) * 100 AS cashback_ratio_without_gst
                FROM cashback_requests cr 
                JOIN users u ON cr.user_id = u.id
                LEFT JOIN users h ON u.head_id = h.id
                WHERE (cr.status = 'Finance Approved' OR cr.status = 'Validator Approved')
                AND cr.created_at >= DATE_SUB(CURRENT_DATE, INTERVAL 12 MONTH)" . $finance_condition . 
                $filter_conditions . "
...
// Get department-wise statistics with filters
 $dept_sql = "SELECT 
               ...
               JOIN users u ON cr.user_id = u.id 
               LEFT JOIN users h ON u.head_id = h.id
               WHERE (cr.status = 'Finance Approved' OR cr.status = 'Validator Approved')" . $finance_condition . $filter_conditions . "
...
// Get day-wise statistics for the last 30 days
 $day_sql = "SELECT 
             ...
             LEFT JOIN users h ON u.head_id = h.id
             WHERE (cr.status = 'Finance Approved' OR cr.status = 'Validator Approved')
             AND cr.created_at >= DATE_SUB(CURRENT_DATE, INTERVAL 30 DAY)" . $finance_condition . $filter_conditions . "
...
// Handle null values for average approval times
if ($avg_approval['avg_manager_hours'] === null) $avg_approval['avg_manager_hours'] = 0;
if ($avg_approval['avg_head_hours'] === null) $avg_approval['avg_head_hours'] = 0;
if ($avg_approval['avg_validator_hours'] === null) $avg_approval['avg_validator_hours'] = 0;
if ($avg_approval['avg_finance_hours'] === null) $avg_approval['avg_finance_hours'] = 0;
...
// Get validators for filter dropdown mapped under those heads
 $validators_sql = "SELECT DISTINCT v.id, v.full_name FROM users v JOIN users h ON v.head_id = h.id WHERE v.role = 'Validator' AND h.finance_id = '" . mysqli_real_escape_string($conn, $current_finance_id) . "' ORDER BY v.full_name";
...
<head>
    ...
    <style>
        /* Same CSS as before with modifications */
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <header>
            ...
            <p class="tagline">Comprehensive CB analytics and approval system</p>
            ...
            <div class="kpi-grid">
                <div class="kpi-card primary">
                    ...
                    <div class="kpi-change">
                        ...
                        $prev_sql = "SELECT COUNT(*) AS count FROM cashback_requests cr JOIN users u ON cr.user_id = u.id LEFT JOIN users h ON u.head_id = h.id
                                     WHERE h.finance_id = '$current_finance_id' AND cr.created_at BETWEEN '$prev_start_date 00:00:00' AND '$prev_end_date 23:59:59'";
                ...
                <div class="kpi-card warning">
                    ...
                    <div class="kpi-change">
                        <?php
                        $prev_pending_sql = "SELECT COUNT(*) AS count FROM cashback_requests cr JOIN users u ON cr.user_id = u.id LEFT JOIN users h ON u.head_id = h.id
                                            WHERE h.finance_id = '$current_finance_id' AND cr.status = 'Validator Approved'
                ...
                <div class="kpi-card success">
                    ...
                    <div class="kpi-change">
                        <?php
                        $prev_approved_sql = "SELECT COUNT(*) AS count FROM cashback_requests cr JOIN users u ON cr.user_id = u.id LEFT JOIN users h ON u.head_id = h.id
                                             WHERE h.finance_id = '$current_finance_id' AND cr.status = 'Finance Approved'
                                             AND cr.created_at BETWEEN '$prev_start_date 00:00:00' AND '$prev_end_date 23:59:59'";
                ...
                <div class="kpi-card danger">
                    ...
                    <div class="kpi-change">
                        <?php
                        $prev_rejected_sql = "SELECT COUNT(*) AS count FROM cashback_requests cr JOIN users u ON cr.user_id = u.id LEFT JOIN users h ON u.head_id = h.id
                                             WHERE h.finance_id = '$current_finance_id' AND (cr.status = 'Rejected' OR cr.status = 'Validator Rejected')
                                             AND cr.created_at BETWEEN '$prev_start_date 00:00:00' AND '$prev_end_date 23:59:59'";
                ...
                <div class="kpi-card info">
                    ...
                    <div class="kpi-value"><?php echo number_format($stats['cashback_ratio_with_gst'], 2); ?>%</div>
                    <div class="kpi-label">Cashback Ratio (with GST)</div>
                    <div class="kpi-change">
                        <?php
                        $prev_ratio_sql = "SELECT 
                                          ...
                                          JOIN users u ON cr.user_id = u.id
                                          LEFT JOIN users h ON u.head_id = h.id
                                          WHERE h.finance_id = '$current_finance_id' AND cr.status IN ('Finance Approved', 'Validator Approved')
                                          AND cr.created_at BETWEEN '$prev_start_date 00:00:00' AND '$prev_end_date 23:59:59'";
                        $prev_ratio_result = mysqli_query($conn, $prev_ratio_sql);
                        $prev_ratio_data = mysqli_fetch_assoc($prev_ratio_result);
                        $ratio_change = $stats['cashback_ratio_with_gst'] - $prev_ratio_data['ratio'];
                ...
                <div class="kpi-card secondary">
                    <i class="fas fa-percentage kpi-icon"></i>
                    <div class="kpi-value"><?php echo number_format($stats['cashback_ratio_without_gst'], 2); ?>%</div>
                    <div class="kpi-label">Cashback Ratio (without GST)</div>
                    <div class="kpi-change">
                        <?php
                        $prev_ratio_sql = "SELECT 
                                          SUM(cr.referral_amount) / NULLIF(SUM(cr.without_gst), 0) * 100 AS ratio
                                          FROM cashback_requests cr
                                          JOIN users u ON cr.user_id = u.id
                                          LEFT JOIN users h ON u.head_id = h.id
                                          WHERE h.finance_id = '$current_finance_id' AND cr.status IN ('Finance Approved', 'Validator Approved')
                        ...
                        $prev_ratio_result = mysqli_query($conn, $prev_ratio_sql);
                        $prev_ratio_data = mysqli_fetch_assoc($prev_ratio_result);
                        $ratio_change = $stats['cashback_ratio_without_gst'] - $prev_ratio_data['ratio'];
                        $ratio_change_percent = $prev_ratio_data['ratio'] > 0 ? round(($ratio_change / $prev_ratio_data['ratio']) * 100, 1) : 0;
                        echo $ratio_change >= 0 ? '+' : '';
                        echo $ratio_change_percent . '%';
                        ?>
                        ...
                        <table class="data-table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="ratio">CB Ratio (with GST)</th>
                                    <th class="ratio">CB Ratio (without GST)</th>
                                    <th class="amount">Total CB</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $dept['cashback_ratio_with_gst'] > 15 ? 'high' : 
                                                     ($dept['cashback_ratio_with_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($dept['cashback_ratio_with_gst'], 2); ?>%
                                            </span>
                                        </td>
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $dept['cashback_ratio_without_gst'] > 15 ? 'high' : 
                                                     ($dept['cashback_ratio_without_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($dept['cashback_ratio_without_gst'], 2); ?>%
                                            </span>
                                        </td>
                            ...
                            <tbody>
                                ...
                                <?php while ($head = mysqli_fetch_assoc($head_result)): ?>
                                    <tr>
                                        ...
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $head['cashback_ratio_with_gst'] > 15 ? 'high' : 
                                                     ($head['cashback_ratio_with_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($head['cashback_ratio_with_gst'], 2); ?>%
                                            </span>
                                        </td>
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $head['cashback_ratio_without_gst'] > 15 ? 'high' : 
                                                     ($head['cashback_ratio_without_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($head['cashback_ratio_without_gst'], 2); ?>%
                                            </span>
                                        </td>
                            ...
                            <tbody>
                                ...
                                <?php while ($manager = mysqli_fetch_assoc($manager_result)): ?>
                                    <tr>
                                        ...
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $manager['cashback_ratio_with_gst'] > 15 ? 'high' : 
                                                     ($manager['cashback_ratio_with_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($manager['cashback_ratio_with_gst'], 2); ?>%
                                            </span>
                                        </td>
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $manager['cashback_ratio_without_gst'] > 15 ? 'high' : 
                                                     ($manager['cashback_ratio_without_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($manager['cashback_ratio_without_gst'], 2); ?>%
                                            </span>
                                        </td>
                            ...
                            <tbody>
                                ...
                                <?php while ($rm = mysqli_fetch_assoc($rm_result)): ?>
                                    <tr>
                                        ...
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $rm['cashback_ratio_with_gst'] > 15 ? 'high' : 
                                                     ($rm['cashback_ratio_with_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($rm['cashback_ratio_with_gst'], 2); ?>%
                                            </span>
                                        </td>
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $rm['cashback_ratio_without_gst'] > 15 ? 'high' : 
                                                     ($rm['cashback_ratio_without_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($rm['cashback_ratio_without_gst'], 2); ?>%
                                            </span>
                                        </td>
                            ...
                            <tbody>
                                ...
                                <?php while ($insurance = mysqli_fetch_assoc($insurance_result)): ?>
                                    <tr>
                                        ...
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $insurance['cashback_ratio_with_gst'] > 15 ? 'high' : 
                                                     ($insurance['cashback_ratio_with_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($insurance['cashback_ratio_with_gst'], 2); ?>%
                                            </span>
                                        </td>
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $insurance['cashback_ratio_without_gst'] > 15 ? 'high' : 
                                                     ($insurance['cashback_ratio_without_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($insurance['cashback_ratio_without_gst'], 2); ?>%
                                            </span>
                                        </td>
                            ...
                            <tbody>
                                ...
                                <?php while ($policy = mysqli_fetch_assoc($policy_result)): ?>
                                    <tr>
                                        ...
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $policy['cashback_ratio_with_gst'] > 15 ? 'high' : 
                                                     ($policy['cashback_ratio_with_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($policy['cashback_ratio_with_gst'], 2); ?>%
                                            </span>
                                        </td>
                                        <td class="ratio">
                                            <span class="ratio-badge <?php 
                                                echo $policy['cashback_ratio_without_gst'] > 15 ? 'high' : 
                                                     ($policy['cashback_ratio_without_gst'] > 10 ? 'medium' : 'low'); 
                                            ?>">
                                                <?php echo number_format($policy['cashback_ratio_without_gst'], 2); ?>%
                                            </span>
                                        </td>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const ratiosWithGst = monthlyData.map(item => item.cashback_ratio_with_gst);
            const ratiosWithoutGst = monthlyData.map(item => item.cashback_ratio_without_gst);
            ...
            new Chart(ratioCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        label: 'Cashback Ratio (without GST) %',
                        data: ratiosWithoutGst,
                        backgroundColor: 'rgba(153, 102, 255, 0.2)',
                        borderColor: 'rgba(153, 102, 255, 1)',
                        borderWidth: 2,
                        fill: true,
